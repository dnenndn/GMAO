...
namespace GMAO
{
	...
	public partial class pareto_cout_intervenant : Form
	{
		// Token: 0x06000AE6 RID: 2790 RVA: 0x001A8A20 File Offset: 0x001A6C20
		public pareto_cout_intervenant()
		{
			this.InitializeComponent();
			this.radGridView3.ViewCellFormatting += new CellFormattingEventHandler(pareto_cout_intervenant.select_change);
			this.radGridView3.ViewRowFormatting += new RowFormattingEventHandler(pareto_cout_intervenant.select_changee);
		}

		// Token: 0x06000AE7 RID: 2791 RVA: 0x001A8A74 File Offset: 0x001A6C74
		...
		// Token: 0x06000AE8 RID: 2792 RVA: 0x001A8AF8 File Offset: 0x001A6CF8
		...
		// Token: 0x06000AE9 RID: 2793 RVA: 0x001A8BFB File Offset: 0x001A6DFB
		private void pareto_cout_intervenant_Load(object sender, EventArgs e)
		{
			this.select_fonction();
			this.select_cout_mo(ordre_travail.id_ort);
		}

		// Token: 0x06000AEA RID: 2794 RVA: 0x001A8C14 File Offset: 0x001A6E14
		public void select_cout_mo(List<int> id_ot)
		{
			...
			string cmdText = "select sum(cout_horaire * (datediff(MINUTE,debut, fin) / 60.0)),intervenant.id, intervenant.nom, sum(datediff(MINUTE,debut, fin) / 60.0) from ot_ordo_intervenant inner join intervenant on ot_ordo_intervenant.id_intervenant = intervenant.id  where id_ressource_fonction in(select ot_ressources_fonction.id from ot_ressources_fonction inner join ordre_travail on ot_ressources_fonction.id_ot = ordre_travail.id where id_ot in (" + str + ") and ordre_travail.statut = @s and (id_fonction = @f or @f = @zero)) group by intervenant.id, intervenant.nom order by sum(cout_horaire * (datediff(MINUTE,debut, fin) / 60.0)) desc ";
			...
			if (flag)
			{
				...
				if (isChecked)
				{
					...
					this.FillColors(this.radChartView1.View, KnownPalette.Spring);
		...
		// Token: 0x06000AEB RID: 2795 RVA: 0x001A90EE File Offset: 0x001A72EE
		private void radRadioButton7_ToggleStateChanged(object sender, StateChangedEventArgs args)
		{
			this.select_cout_mo(ordre_travail.id_ort);
		}

		// Token: 0x06000AEC RID: 2796 RVA: 0x001A90FD File Offset: 0x001A72FD
		private void radRadioButton8_ToggleStateChanged(object sender, StateChangedEventArgs args)
		{
			this.select_cout_mo(ordre_travail.id_ort);
		}

		// Token: 0x06000AED RID: 2797 RVA: 0x001A910C File Offset: 0x001A730C
		...
		// Token: 0x06000AEE RID: 2798 RVA: 0x001A9120 File Offset: 0x001A7320
		...
		// Token: 0x06000AEF RID: 2799 RVA: 0x001A9256 File Offset: 0x001A7456
