...
namespace GMAO
{
	...
	public partial class pareto_fiabilite_equipement : Form
	{
		// Token: 0x06000B1A RID: 2842 RVA: 0x001AE618 File Offset: 0x001AC818
		public pareto_fiabilite_equipement()
		{
			this.InitializeComponent();
			this.radChartView1.ShowTrackBall = true;
		}

		// Token: 0x06000B1B RID: 2843 RVA: 0x001AE640 File Offset: 0x001AC840
		private void pareto_fiabilite_equipement_Load(object sender, EventArgs e)
		{
		...
		// Token: 0x06000B1C RID: 2844 RVA: 0x001AE6C4 File Offset: 0x001AC8C4
		private void select_pareto_disponibilite()
		{
			bd bd = new bd();
			this.radChartView1.Series.Clear();
			string str = string.Join<int>(",", ordre_travail.id_ort.ToArray());
			string cmdText = "with temporaire as(select ordre_travail.id i,concat(date_debut,@espace, heure_debut, @wa) debut , concat(date_fin,@espace, heure_fin, @wa) fin, datediff(MINUTE,concat(date_debut,@espace, heure_debut, @wa),concat(date_fin,@espace, heure_fin, @wa))/60.0 d, ordre_travail.equipement e, equipement.code c, equipement.designation desi, equipement.deleted dl from ordre_travail inner join equipement on ordre_travail.equipement = equipement.id where ordre_travail.id in (" + str + ") and ordre_travail.statut = @a and ordre_travail.type not like '%'  + @p + '%') select (sum(d) * 100) / (select sum(d) from temporaire), e, c, desi, sum(d) ttr,avg(d) mttr, count(i) nbre_ot from temporaire  group by e,c,desi order by sum(d) desc";
		...
		// Token: 0x06000B1D RID: 2845 RVA: 0x001AEA91 File Offset: 0x001ACC91
		...
		// Token: 0x06000B1E RID: 2846 RVA: 0x001AEAA4 File Offset: 0x001ACCA4
		private void select_pareto_maintenabilite()
		{
			bd bd = new bd();
			this.radChartView1.Series.Clear();
			string str = string.Join<int>(",", ordre_travail.id_ort.ToArray());
			string cmdText = "with temporaire as(select ordre_travail.id i,concat(date_debut,@espace, heure_debut, @wa) debut , concat(date_fin,@espace, heure_fin, @wa) fin, datediff(MINUTE,concat(date_debut,@espace, heure_debut, @wa),concat(date_fin,@espace, heure_fin, @wa))/60.0 d, ordre_travail.equipement e, equipement.code c, equipement.designation desi, equipement.deleted dl from ordre_travail inner join equipement on ordre_travail.equipement = equipement.id where ordre_travail.id in (" + str + ") and ordre_travail.statut = @a and ordre_travail.type not like '%'  + @p + '%') select (avg(d) * count(i) * 100) / (select sum(d) from temporaire), e, c, desi, sum(d) ttr,avg(d) mttr, count(i) nbre_ot from temporaire  group by e,c,desi order by avg(d) desc";
			SqlCommand sqlCommand = new SqlCommand(cmdText, bd.cnn);
			sqlCommand.Parameters.Add("@a", SqlDbType.VarChar).Value = "Clôturé";
			sqlCommand.Parameters.Add("@p", SqlDbType.VarChar).Value = "Préventive";
			sqlCommand.Parameters.Add("@espace", SqlDbType.VarChar).Value = " ";
		...
		// Token: 0x06000B1F RID: 2847 RVA: 0x001AEE74 File Offset: 0x001AD074
		private void select_pareto_fiabilite()
		{
			bd bd = new bd();
			this.radChartView1.Series.Clear();
			string str = string.Join<int>(",", ordre_travail.id_ort.ToArray());
			string cmdText = "with temporaire as(select ordre_travail.id i,concat(date_debut,@espace, heure_debut, @wa) debut , concat(date_fin,@espace, heure_fin, @wa) fin, datediff(MINUTE,concat(date_debut,@espace, heure_debut, @wa),concat(date_fin,@espace, heure_fin, @wa))/60.0 d, ordre_travail.equipement e, equipement.code c, equipement.designation desi, equipement.deleted dl from ordre_travail inner join equipement on ordre_travail.equipement = equipement.id where ordre_travail.id in (" + str + ") and ordre_travail.statut = @a and ordre_travail.type not like '%'  + @p + '%') select (count(i) * 100) / (select count(i) from temporaire), e, c, desi, sum(d) ttr,avg(d) mttr, count(i) nbre_ot from temporaire  group by e,c,desi order by count(i) desc";
			SqlCommand sqlCommand = new SqlCommand(cmdText, bd.cnn);
			sqlCommand.Parameters.Add("@a", SqlDbType.VarChar).Value = "Clôturé";
			sqlCommand.Parameters.Add("@p", SqlDbType.VarChar).Value = "Préventive";
			sqlCommand.Parameters.Add("@espace", SqlDbType.VarChar).Value = " ";
		...
		// Token: 0x06000B20 RID: 2848 RVA: 0x001AF228 File Offset: 0x001AD428
		...
		// Token: 0x06000B21 RID: 2849 RVA: 0x001AF250 File Offset: 0x001AD450
		...
		// Token: 0x06000B22 RID: 2850 RVA: 0x001AF278 File Offset: 0x001AD478
		...
		// Token: 0x06000B23 RID: 2851 RVA: 0x001AF2A0 File Offset: 0x001AD4A0
		...
		// Token: 0x06000B24 RID: 2852 RVA: 0x001AF304 File Offset: 0x001AD504
		private void select_pareto_combine()
		{
			...
			if (flag)
			{
				...
				{
					string str = string.Join<int>(",", ordre_travail.id_ort.ToArray());
					string cmdText = "with temporaire as(select ordre_travail.id i,concat(date_debut,@espace, heure_debut, @wa) debut , concat(date_fin,@espace, heure_fin, @wa) fin, datediff(MINUTE,concat(date_debut,@espace, heure_debut, @wa),concat(date_fin,@espace, heure_fin, @wa))/60.0 d, ordre_travail.equipement e, equipement.code c, equipement.designation desi, equipement.deleted dl from ordre_travail inner join equipement on ordre_travail.equipement = equipement.id where ordre_travail.id in (" + str + ") and ordre_travail.statut = @a and ordre_travail.type not like '%'  + @p + '%') select ((@v1 * count(i) + @v2 * sum(d) + @v3 * avg(d)*count(i)) * 100) / (select @v1*count(i) + @v2 * sum(d) + @v3 * sum(d) from temporaire), e, c, desi, sum(d) ttr,avg(d) mttr, count(i) nbre_ot, @v1 * count(i) + @v2 * sum(d) + @v3 * avg(d) from temporaire  group by e,c,desi order by @v1 * count(i) + @v2 * sum(d) + @v3 * avg(d)*count(i) desc";
					SqlCommand sqlCommand = new SqlCommand(cmdText, bd.cnn);
					sqlCommand.Parameters.Add("@a", SqlDbType.VarChar).Value = "Clôturé";
					sqlCommand.Parameters.Add("@p", SqlDbType.VarChar).Value = "Préventive";
					sqlCommand.Parameters.Add("@espace", SqlDbType.VarChar).Value = " ";
		...
		// Token: 0x06000B25 RID: 2853 RVA: 0x001AF802 File Offset: 0x001ADA02
