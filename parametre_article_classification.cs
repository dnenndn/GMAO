...
namespace GMAO
{
	...
	public partial class parametre_article_classification : Form
	{
		// Token: 0x06000EF6 RID: 3830 RVA: 0x002410F4 File Offset: 0x0023F2F4
		public parametre_article_classification()
		{
		...
		// Token: 0x06000EF7 RID: 3831 RVA: 0x0024116A File Offset: 0x0023F36A
		private void parametre_article_classification_Load(object sender, EventArgs e)
		{
			this.remplissage_tableau(0);
		}

		// Token: 0x06000EF8 RID: 3832 RVA: 0x00241178 File Offset: 0x0023F378
		private void remplissage_tableau(int o)
		{
			...
			string cmdText = "select id,code_classification, designation from parametre_classification_article where deleted = @i";
			bd bd = new bd();
			...
			if (flag2)
			{
				...
				if (flag5)
				{
					this.radGridView1.Rows[parametre_article_classification.row_act].IsCurrent = true;
				}
				bool flag6 = o == 3;
				if (flag6)
				{
					bool flag7 = parametre_article_classification.row_act != 0;
					if (flag7)
					{
						this.radGridView1.Rows[parametre_article_classification.row_act - 1].IsCurrent = true;
					}
					else
					{
						this.radGridView1.MasterTemplate.MoveToFirstPage();
		...
		// Token: 0x06000EF9 RID: 3833 RVA: 0x00241474 File Offset: 0x0023F674
		private void action_tableau(object sender, GridViewCellEventArgs e)
		{
			...
			if (flag)
			{
				...
				if (flag2)
				{
					...
					parametre_article_classification.row_act = e.RowIndex;
					...
					if (flag3)
					{
						...
						if (flag4)
						{
							string cmdText = "update parametre_classification_article set deleted = @d where id = @a";
							bd bd = new bd();
							SqlCommand sqlCommand = new SqlCommand(cmdText, bd.cnn);
					...
					if (flag5)
					{
						parametre_article_classification.id_modifier = value;
						this.guna2Button1.Text = "Modifier";
						this.label2.Text = "Modifier Classification";
		...
		// Token: 0x06000EFA RID: 3834 RVA: 0x00241664 File Offset: 0x0023F864
		private void guna2Button1_Click(object sender, EventArgs e)
		{
			...
			if (flag)
			{
				...
				if (flag2)
				{
					string cmdText = "select id from parametre_classification_article where (code_classification = @v1 or designation = @v2) and deleted = @d";
					bd bd = new bd();
					SqlCommand sqlCommand = new SqlCommand(cmdText, bd.cnn);
					...
					if (flag3)
					{
						string cmdText2 = "insert into parametre_classification_article (code_classification, designation, deleted) values (@i1, @i2, @i3)";
						...
						bd.cnn.Close();
						MessageBox.Show("Classification est ajoutée avec succés");
						this.TextBox1.Clear();
						this.TextBox2.Clear();
					...
					{
						MessageBox.Show("Erreur : Classification déja existe", "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Hand);
					}
				}
				bool flag4 = this.guna2Button1.Text == "Modifier";
				if (flag4)
				{
					string cmdText3 = "select id from parametre_classification_article where (code_classification = @v1 or designation = @v2) and deleted = @d and id <> @i";
					bd bd2 = new bd();
					...
					sqlCommand3.Parameters.Add("@i", SqlDbType.Int).Value = parametre_article_classification.id_modifier;
					...
					if (flag5)
					{
						string cmdText4 = "update parametre_classification_article set code_classification = @i1, designation = @i2 where id = @i";
						...
						sqlCommand4.Parameters.Add("@i", SqlDbType.Int).Value = parametre_article_classification.id_modifier;
						bd2.ouverture_bd(bd2.cnn);
						sqlCommand4.ExecuteNonQuery();
						bd2.cnn.Close();
						MessageBox.Show("Classification est modifié avec succés");
						this.TextBox1.Clear();
						this.TextBox2.Clear();
						this.guna2Button1.Text = "Ajouter";
						this.label2.Text = "Ajouter Classification";
