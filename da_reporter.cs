...
namespace GMAO
{
	...
	public partial class da_reporter : Form
	{
		// Token: 0x06000498 RID: 1176 RVA: 0x000C37C0 File Offset: 0x000C19C0
		public da_reporter()
		{
		...
		// Token: 0x06000499 RID: 1177 RVA: 0x000C385B File Offset: 0x000C1A5B
		private void da_reporter_Load(object sender, EventArgs e)
		{
			this.remplissage_tableau(0);
		}

		// Token: 0x0600049A RID: 1178 RVA: 0x000C3868 File Offset: 0x000C1A68
		private void radGridView1_CellFormatting(object sender, CellFormattingEventArgs e)
		{
			bool flag = e.CellElement.ColumnInfo is GridViewCommandColumn & e.CellElement.RowIndex != -1 & e.CellElement.ColumnIndex == 8;
			if (flag)
			{
		...
		// Token: 0x0600049B RID: 1179 RVA: 0x000C3924 File Offset: 0x000C1B24
		...
		// Token: 0x0600049C RID: 1180 RVA: 0x000C3A38 File Offset: 0x000C1C38
		...
		// Token: 0x0600049D RID: 1181 RVA: 0x000C3E94 File Offset: 0x000C2094
			...
			if (flag)
			{
				...
				if (flag2)
				{
					...
					if (flag3)
					{
						...
						da_reporter.row_act = e.RowIndex;
		...
		// Token: 0x0600049E RID: 1182 RVA: 0x000C40D8 File Offset: 0x000C22D8
		private void remplissage_tableau(int o)
		{
			...
			if (flag)
			{
				string cmdText = "select demande_achat.id, date_da, heure_da, createur, demandeur, delai, periode,da_reporter.reporter_par, da_reporter.nouvel_date from demande_achat inner join da_reporter on demande_achat.id = da_reporter.id_da where statut = @i";
				bd bd = new bd();
				SqlCommand sqlCommand = new SqlCommand(cmdText, bd.cnn);
				sqlCommand.Parameters.Add("@i", SqlDbType.Int).Value = 4;
				SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(sqlCommand);
				DataTable dataTable = new DataTable();
				sqlDataAdapter.Fill(dataTable);
				...
				if (flag3)
				{
					for (int i = 0; i < dataTable.Rows.Count; i++)
					{
						this.radGridView1.Rows.Add(new object[]
						{
							...
							this.select_utilisateur(dataTable.Rows[i].ItemArray[7].ToString()),
							fonction.Mid(dataTable.Rows[i].ItemArray[8].ToString(), 1, 10)
						});
						this.radGridView1.Rows[i].Cells[8].Value = "Retourner en cours";
						this.radGridView1.Rows[i].Cells[9].Value = this.imageList1.Images[10];
					}
				}
			}
			else
			{
				string cmdText2 = "select demande_achat.id, date_da, heure_da, createur, demandeur, delai, periode , periode,da_reporter.reporter_par, da_reporter.nouvel_date from demande_achat inner join da_reporter on demande_achat.id = da_reporter.id_da where statut = @i and demandeur = @v";
				bd bd2 = new bd();
				SqlCommand sqlCommand2 = new SqlCommand(cmdText2, bd2.cnn);
				sqlCommand2.Parameters.Add("@i", SqlDbType.Int).Value = 4;
				...
				if (flag4)
				{
					for (int j = 0; j < dataTable2.Rows.Count; j++)
					{
						this.radGridView1.Rows.Add(new object[]
						{
							...
							this.select_utilisateur(dataTable2.Rows[j].ItemArray[7].ToString()),
							fonction.Mid(dataTable2.Rows[j].ItemArray[8].ToString(), 1, 10)
						});
						this.radGridView1.Rows[j].Cells[8].Value = "Retourner en cours";
						this.radGridView1.Rows[j].Cells[9].Value = this.imageList1.Images[10];
					}
				}
			}
			...
			if (flag5)
			{
				...
				if (flag8)
				{
					this.radGridView1.Rows[da_reporter.row_act].IsCurrent = true;
				}
				bool flag9 = o == 3;
				if (flag9)
				{
					bool flag10 = da_reporter.row_act != 0;
					if (flag10)
					{
						this.radGridView1.Rows[da_reporter.row_act - 1].IsCurrent = true;
					}
					else
					{
						this.radGridView1.MasterTemplate.MoveToFirstPage();
		...
		// Token: 0x0600049F RID: 1183 RVA: 0x000C47D0 File Offset: 0x000C29D0
