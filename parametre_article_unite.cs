...
namespace GMAO
{
	...
	public partial class parametre_article_unite : Form
	{
		// Token: 0x06000F0B RID: 3851 RVA: 0x00245618 File Offset: 0x00243818
		public parametre_article_unite()
		{
		...
		// Token: 0x06000F0C RID: 3852 RVA: 0x0024568E File Offset: 0x0024388E
		private void parametre_article_unite_Load(object sender, EventArgs e)
		{
			this.remplissage_tableau(0);
		}

		// Token: 0x06000F0D RID: 3853 RVA: 0x0024569C File Offset: 0x0024389C
		private void remplissage_tableau(int o)
		{
			...
			string cmdText = "select id,code_unite, designation from parametre_unite_article where deleted = @i";
			bd bd = new bd();
			...
			if (flag2)
			{
				...
				if (flag5)
				{
					this.radGridView1.Rows[parametre_article_unite.row_act].IsCurrent = true;
				}
				bool flag6 = o == 3;
				if (flag6)
				{
					bool flag7 = parametre_article_unite.row_act != 0;
					if (flag7)
					{
						this.radGridView1.Rows[parametre_article_unite.row_act - 1].IsCurrent = true;
					}
					else
					{
						this.radGridView1.MasterTemplate.MoveToFirstPage();
		...
		// Token: 0x06000F0E RID: 3854 RVA: 0x00245998 File Offset: 0x00243B98
		private void action_tableau(object sender, GridViewCellEventArgs e)
		{
			...
			if (flag)
			{
				...
				if (flag2)
				{
					...
					parametre_article_unite.row_act = e.RowIndex;
					...
					if (flag3)
					{
						...
						if (flag4)
						{
							string cmdText = "update parametre_unite_article set deleted = @d where id = @a";
							bd bd = new bd();
							SqlCommand sqlCommand = new SqlCommand(cmdText, bd.cnn);
					...
					if (flag5)
					{
						parametre_article_unite.id_modifier = value;
						this.guna2Button1.Text = "Modifier";
						this.label2.Text = "Modifier Unité de stockage";
		...
		// Token: 0x06000F0F RID: 3855 RVA: 0x00245B88 File Offset: 0x00243D88
		private void guna2Button1_Click(object sender, EventArgs e)
		{
			...
			if (flag)
			{
				...
				if (flag2)
				{
					string cmdText = "select id from parametre_unite_article where (code_unite = @v1 or designation = @v2) and deleted = @d";
					bd bd = new bd();
					SqlCommand sqlCommand = new SqlCommand(cmdText, bd.cnn);
					...
					if (flag3)
					{
						string cmdText2 = "insert into parametre_unite_article (code_unite, designation, deleted) values (@i1, @i2, @i3)";
						...
						bd.cnn.Close();
						MessageBox.Show("L'unité de stockage est ajoutée avec succés");
						this.TextBox1.Clear();
						this.TextBox2.Clear();
					...
					{
						MessageBox.Show("Erreur : L'unité de stockage déja existe", "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Hand);
					}
				}
				bool flag4 = this.guna2Button1.Text == "Modifier";
				if (flag4)
				{
					string cmdText3 = "select id from parametre_unite_article where (code_unite = @v1 or designation = @v2) and deleted = @d and id <> @i";
					bd bd2 = new bd();
					...
					sqlCommand3.Parameters.Add("@i", SqlDbType.Int).Value = parametre_article_unite.id_modifier;
					...
					if (flag5)
					{
						string cmdText4 = "update parametre_unite_article set code_unite = @i1, designation = @i2 where id = @i";
						...
						sqlCommand4.Parameters.Add("@i", SqlDbType.Int).Value = parametre_article_unite.id_modifier;
						bd2.ouverture_bd(bd2.cnn);
						sqlCommand4.ExecuteNonQuery();
						bd2.cnn.Close();
						MessageBox.Show("L'unité de stockage est modifié avec succés");
						this.TextBox1.Clear();
						this.TextBox2.Clear();
						this.guna2Button1.Text = "Ajouter";
						this.label2.Text = "Ajouter Unité de stockage";
