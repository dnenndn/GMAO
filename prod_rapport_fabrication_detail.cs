...
namespace GMAO
{
	...
	public partial class prod_rapport_fabrication_detail : Form
	{
		// Token: 0x06000C45 RID: 3141 RVA: 0x001DF518 File Offset: 0x001DD718
		public prod_rapport_fabrication_detail()
		{
		...
		// Token: 0x06000C46 RID: 3142 RVA: 0x001DF56B File Offset: 0x001DD76B
		private void prod_rapport_fabrication_detail_Load(object sender, EventArgs e)
		{
			this.select_champs();
		}

		// Token: 0x06000C47 RID: 3143 RVA: 0x001DF578 File Offset: 0x001DD778
		private void select_champs()
		{
			bd bd = new bd();
			this.radGridView1.Rows.Clear();
			string cmdText = "select prod_rapport_fabrication_qualite.id, prod_saisie.designation, valeur from prod_rapport_fabrication_qualite inner join prod_saisie on prod_rapport_fabrication_qualite.id_qualite = prod_saisie.id where id_rapport = @i1 and id_unite = @i2";
		...
		// Token: 0x06000C48 RID: 3144 RVA: 0x001DF6B8 File Offset: 0x001DD8B8
		private void guna2Button1_Click(object sender, EventArgs e)
		{
			...
			if (flag)
			{
				...
				if (flag3)
				{
					...
					{
						string cmdText = "update prod_rapport_fabrication_qualite set valeur = @h where id = @i";
						SqlCommand sqlCommand = new SqlCommand(cmdText, bd.cnn);
						...
						bd.cnn.Close();
						string cmdText2 = "select id_mesure from prod_mesure where id_unite = @i1 and id_saisie in(select id_qualite from prod_rapport_fabrication_qualite where id = @i2)";
