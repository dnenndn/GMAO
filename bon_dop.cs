...
namespace GMAO
{
	...
	public partial class bon_dop : Form
	{
		// Token: 0x0600023F RID: 575 RVA: 0x00061F69 File Offset: 0x00060169
		public bon_dop()
		{
		...
		// Token: 0x06000240 RID: 576 RVA: 0x00061F88 File Offset: 0x00060188
		private void bon_dop_Load(object sender, EventArgs e)
		{
		}

		// Token: 0x06000241 RID: 577 RVA: 0x00061F8C File Offset: 0x0006018C
		private void test()
		{
			bd bd = new bd();
			...
			sqlCommand.Parameters.Add("@i", SqlDbType.VarChar).Value = choisir_fournisseur.nom_fournisseur;
			SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(sqlCommand);
			DataTable dataTable = new DataTable();
			sqlDataAdapter.Fill(dataTable);
			string cmdText2 = "select dop_fournisseur.id_dop from dop_fournisseur inner join fournisseur on dop_fournisseur.id_fournisseur = fournisseur.id and fournisseur.nom = @i2 and dop_fournisseur.id_dop < @i3";
			SqlCommand sqlCommand2 = new SqlCommand(cmdText2, bd.cnn);
			sqlCommand2.Parameters.Add("@i2", SqlDbType.VarChar).Value = choisir_fournisseur.nom_fournisseur;
			sqlCommand2.Parameters.Add("@i3", SqlDbType.Int).Value = choisir_fournisseur.id_dop;
			...
			Span span2 = new Span(choisir_fournisseur.nom_fournisseur ?? "");
			...
			string cmdText3 = "select mail, telephone from utilisateur where id = @l";
			SqlCommand sqlCommand3 = new SqlCommand(cmdText3, bd.cnn);
			sqlCommand3.Parameters.Add("@l", SqlDbType.Int).Value = page_loginn.id_user;
			...
			if (flag)
			{
				str = Convert.ToString(dataTable3.Rows[0].ItemArray[0].ToString());
				str2 = Convert.ToString(dataTable3.Rows[0].ItemArray[1].ToString());
			}
			...
			string str3 = "N° : ";
			object obj = dataTable.Rows[0].ItemArray[3];
			Span span4 = new Span(str3 + ((obj != null) ? obj.ToString() : null) + "-D-" + num.ToString());
			...
			string cmdText4 = "select article.code_article, article.designation, qt  from dop_article inner join article on dop_article.id_article = article.id  where id_dop = @i";
			SqlCommand sqlCommand4 = new SqlCommand(cmdText4, bd.cnn);
			sqlCommand4.Parameters.Add("@i", SqlDbType.Int).Value = choisir_fournisseur.id_dop;
			...
			if (flag2)
			{
				...
				{
					string h = dataTable4.Rows[i].ItemArray[0].ToString();
					string h2 = dataTable4.Rows[i].ItemArray[1].ToString();
					string h3 = dataTable4.Rows[i].ItemArray[2].ToString();
					this.remplissage_tableau(table2, h, h2, h3);
				}
			}
		...
		// Token: 0x06000242 RID: 578 RVA: 0x00062D68 File Offset: 0x00060F68
		private void remplissage_tableau(Table t, string h1, string h2, string h3)
		{
			TableRow tableRow = new TableRow();
			TableCell tableCell = new TableCell();
			Paragraph paragraph = new Paragraph();
